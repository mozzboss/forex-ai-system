...
import { ALL_PAIRS, getAccountRules, getPricePrecision, TRADING_CONFIG } from "@/config/trading";
...
import { cn, formatCurrency, formatPercent, getKillZoneStatus } from "@/lib/utils";
import type {
  CurrencyPair,
  FullAnalysis,
  NewsEvent,
...
function getExecutionBlockers({
  ...
  // Skip entry status check when the trader manually confirms the trigger
  if (!overrideConfirmation && analysis.entryStatus.status !== "CONFIRMED") {
  ...
  if (denialResult && !denialResult.allowed) {
    // When override is active, skip the no_confirmation denial from the denial engine
    const relevantDenials = overrideConfirmation
      ? denialResult.denials.filter((d) => d.reason !== "no_confirmation")
      : denialResult.denials;
    relevantDenials.forEach((denial) => blockers.push(denial.explanation));
  }
...
export default function PairPage({ params }: { params: { pair: string } }) {
  ...
  const [confirmAccount, setConfirmAccount] = useState<TradingAccount | null>(null);
  ...
  const runAnalysis = async () => {
    ...
    setFeedbackByAccount({});
    await analyze(pair, marketNotes.trim() || undefined);
  ...
  const recordTrade = async (account: TradingAccount) => {
    ...
    const blockers = getExecutionBlockers({
      ...
      executionPrice,
      pair,
      overrideConfirmation: manualOverride,
    });

    if (blockers.length > 0) {
      saveFeedback(account.id, "error", blockers.join(" | "));
      return;
    }
    ...
    try {
      const res = await authFetch("/api/trade", {
        ...
        body: JSON.stringify({
          ...
          status: "open",
          entryStatus: manualOverride ? "CONFIRMED" : analysis.entryStatus.status,
          ...
          notes: buildTradeNotes(
            manualOverride
              ? `[MANUAL OVERRIDE] Trader confirmed trigger on chart. AI status was ${analysis.entryStatus.status}. ${marketNotes}`
              : marketNotes,
            entryPrice
          ),
        }),
      ...
      <section className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.22),_transparent_35%),linear-gradient(135deg,_rgba(15,23,42,1),_rgba(30,41,59,0.96))] p-5 sm:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-3 xl:min-w-[520px]">
            ...
            />
            <HeroMetric label="Min Score" value={`${TRADING_CONFIG.minTradeScore}+/10`} detail="minimum score required across all account types" />
            ...
            <div className="mt-4 rounded-2xl border border-white/10 bg-surface-light p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-3">
                  <KillZoneBadge killZones={TRADING_CONFIG.killZones} />
          ...
          <div className="space-y-4">
            <Card>
              ...
              <div className="mt-4 space-y-2 rounded-xl border border-yellow-500/20 bg-yellow-500/5 p-4">
                ...
                <p className="text-sm text-gray-300">Score below {TRADING_CONFIG.minTradeScore} means deny. All accounts use the same threshold.</p>
                  ...
                  <div key={account.id} className="rounded-2xl border border-white/10 bg-slate-950/25 p-4">
                    ...
                    {analysis ? (
                      <div className="mt-4 border-t border-white/5 pt-4">
                        ...
                        {/* Manual override toggle — only shown when AI says WAIT/READY */}
                        {(analysis.entryStatus.status === "WAIT" || analysis.entryStatus.status === "READY") && analysis.finalDecision.decision === "TAKE_TRADE" ? (
                          <div className={cn(
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <div className={cn(
                                  ...
                                  manualOverride ? "text-orange-300" : "text-slate-500"
                                )}>
                                  Manual Trigger Override
                                </div>
                                <p className="mt-1 text-xs text-slate-400">
                                  {manualOverride
                                    ? "Override active — you confirm the trigger is visible on your MT4 chart."
                                    : `AI says ${analysis.entryStatus.status}. Toggle if you see the trigger on your chart.`}
                                </p>
                              </div>
                              <button
                                type="button"
                                ...
                                className={cn(
                                  "shrink-0 rounded-lg border px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.12em] transition-colors",
                                  manualOverride
                                    ? "border-orange-500/40 bg-orange-500/15 text-orange-200 hover:bg-orange-500/25"
                                    : "border-white/10 bg-surface text-slate-300 hover:border-white/20"
                                )}
                              >
                        ...
                        {confirmAccount?.id === account.id ? (
                          <div className="mt-4 rounded-xl border border-yellow-500/30 bg-yellow-500/5 p-4">
                            <p className="text-sm font-semibold text-yellow-300">Confirm trade recording</p>
                            {manualOverride ? (
                              <p className="mt-1 text-xs font-semibold text-orange-300">
                                Manual override active. You are bypassing the AI&apos;s {analysis.entryStatus.status} status. Only proceed if you have confirmed the entry trigger on your chart.
                              </p>
                            ) : null}
                            <p className="mt-2 text-sm text-gray-300">
                              {analysis.tradeSetup?.direction} {pair} at {executionPrice || "AI entry"} — risk {formatCurrency(analysis.riskPerAccount[account.id]?.riskAmount ?? 0)} on {account.name}. This cannot be undone from here.
                            </p>
                            <div className="mt-3 flex gap-2">
                              <Button
                                onClick={async () => { await recordTrade(account); setConfirmAccount(null); }}
                                disabled={activeActionId !== null}
                                className="w-full sm:w-auto"
                              >
                                {activeActionId === `${account.id}:create` ? "Recording..." : "Confirm"}
                              </Button>
                              <Button variant="ghost" onClick={() => setConfirmAccount(null)} className="w-full sm:w-auto">
                                Cancel
                              </Button>
                            </div>
                          </div>
...
function KillZoneBadge({
  killZones,
}: {
  killZones: Record<string, { start: string; end: string; label: string }>;
}) {
  const status = getKillZoneStatus(killZones);

  if (status.active) {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full border border-green-500/30 bg-green-500/10 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-green-300">
  ...
  if (status.nextLabel) {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full border border-yellow-500/30 bg-yellow-500/10 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-yellow-400">
        <span className="size-1.5 rounded-full bg-yellow-400 opacity-70" />
        Outside kill zone — next: {status.nextLabel} {status.nextStartsAt} UTC
      </span>
    );
  }

  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-500">
      No kill zone today
    </span>
  );
}
