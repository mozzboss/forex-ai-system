 ...
 */
const ZERO_OUTPUT: RiskOutput = {
  riskAmount: 0, pipDistance: 0, lotSize: 0, maxLoss: 0, maxProfit: 0, riskRewardRatio: 0, isValid: false,
};
...
export function calculateRisk(input: RiskInput): RiskOutput {
  ...
  if (!accountBalance || accountBalance <= 0) {
    return { ...ZERO_OUTPUT, denial: "Account balance must be a positive number." };
  }
  if (!riskPercent || riskPercent <= 0) {
    return { ...ZERO_OUTPUT, denial: "Risk percentage must be greater than zero." };
  }
  if (entryPrice === stopLossPrice) {
    return { ...ZERO_OUTPUT, denial: "Stop loss must differ from entry price." };
  }
