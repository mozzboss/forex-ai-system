...
export async function requireAppUserId(req: NextRequest): Promise<string> {
  const accessToken = getBearerToken(req);
  const allowDefaultUserFallback =
    process.env.ALLOW_DEFAULT_USER_FALLBACK === "true" &&
    process.env.NODE_ENV !== "production";

  // Optional local-only fallback for single-user development mode.
  if (!accessToken) {
    if (allowDefaultUserFallback) {
