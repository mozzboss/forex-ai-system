...
const analysisAccountSchema = z.object({
  id: z.string().uuid(),
  ...
  riskPercent: z.number().nonnegative(),
  maxDailyLoss: z.number().nonnegative(),
  maxDrawdown: z.number().nonnegative(),
  maxTradesPerDay: z.number().int().nonnegative(),
  currentDailyLoss: z.number().nonnegative(),
  currentDailyTrades: z.number().int().nonnegative(),
  lossesInARow: z.number().int().nonnegative(),
...
export const analysisRequestSchema = z.object({
  pair: currencyPairSchema,
  accounts: z.array(analysisAccountSchema).min(1),
...
export const mtImportPositionSchema = z.object({
  ...
  stopLoss: z.number().min(0).optional().transform((v) => (v === 0 ? undefined : v)),
  takeProfit: z.number().min(0).optional().transform((v) => (v === 0 ? undefined : v)),
